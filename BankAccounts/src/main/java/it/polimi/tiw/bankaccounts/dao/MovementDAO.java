...
public class MovementDAO {
	...
	public List<Movement> getReceivedMovementsByAccount(int accountId, User user) throws SQLException {
		...
		String query = "SELECT M.*, U.name, U.surname FROM bankaccounts.movement M JOIN bankaccounts.bankaccount A JOIN bankaccounts.bankaccount A2 JOIN bankaccounts.user U "
	...
	public Integer addMovement(int senderAccountId, int receiverAccountId, double amount, String reason)
		...
		String query1 = "INSERT INTO bankaccounts.movement (date, amount, reason, sender_account, receiver_account) VALUES (?, ?, ?, ?, ?);";
		String query2 = "SELECT id FROM bankaccounts.movement WHERE date = ? AND amount = ? AND reason = ? AND sender_account = ? AND receiver_account = ?;";
		ResultSet result;
		PreparedStatement pstatement = null;
	...
	public Movement getMovementDetails(int movementId, User user) throws SQLException {
		...
		String query = "SELECT M.*, U.name AS n1, U.surname AS s1, U2.name AS n2, U2.surname AS s2 "
				+ "FROM bankaccounts.movement M JOIN bankaccounts.bankaccount A JOIN bankaccounts.bankaccount A2 JOIN bankaccounts.user U JOIN bankaccounts.user U2 "
	...
	public BankAccount[] getAccountsByMovement(int movementId, User user) throws SQLException {
		...
		String query = "SELECT A.id AS id1, A.balance AS b1, A.owner AS o1, A2.id AS id2, A2.balance AS b2, A2.owner AS o2 "
				+ "FROM bankaccounts.movement M JOIN bankaccounts.bankaccount A JOIN bankaccounts.bankaccount A2 "
